...
func (b *SimulatedBackend) TransactionReceipt(ctx context.Context, txHash common.Hash) (*types.Receipt, error) {
	...
	blockNumber := rawdb.ReadTxLookupEntry(b.database, txHash)
	if blockNumber == nil {
		return nil, ethereum.NotFound
	}

	stateReader, _ := b.stateByBlockNumber(ctx, new(big.Int).SetUint64(*blockNumber))

	receipt, _, _, _ := rawdb.ReadReceipt(b.database, txHash, b.config, stateReader)
	if receipt == nil {
		return nil, ethereum.NotFound
	}
	return receipt, nil
...
func (b *SimulatedBackend) SendTransaction(ctx context.Context, tx *types.Transaction) error {
	...
	stateReader, _ := b.blockchain.State()
	signer := types.MakeSigner(b.blockchain.Config(), block.Number(), stateReader)
...
func (fb *filterBackend) GetReceipts(ctx context.Context, hash common.Hash) (types.Receipts, error) {
	...
	stateReader, err := fb.backend.stateByBlockNumber(ctx, big.NewInt(int64(*number)))
	if err != nil {
		return nil, err
	}
	return rawdb.ReadReceipts(fb.db, hash, *number, fb.bc.Config(), stateReader), nil
}
...
func (fb *filterBackend) StateAndHeaderByNumber(ctx context.Context, number rpc.BlockNumber) (*state.StateDB, *types.Header, error) {
	...
	return stateReader, fb.bc.GetHeaderByNumber(uint64(number.Int64())), nil
}

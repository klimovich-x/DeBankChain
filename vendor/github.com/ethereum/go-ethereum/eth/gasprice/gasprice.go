...
func (oracle *Oracle) SuggestTipCap(ctx context.Context) (*big.Int, error) {
	...
	stateReader, _ := oracle.backend.StateAt(head.Root)
	for sent < oracle.checkBlocks && number > 0 {
		go oracle.getBlockValues(ctx, types.MakeSigner(oracle.backend.ChainConfig(), big.NewInt(int64(number)), stateReader), number, sampleNumber, oracle.ignorePrice, result, quit)
		sent++
		exp++
		number--
	}

	...
	go func() {
		...
		for {
			...
			case txs := <-txs:
				...
				stateReader, _, _ := api.sys.backend.StateAndHeaderByNumber(context.Background(), rpc.BlockNumber(latest.Number.Int64()))
				for _, tx := range txs {
					if fullTx != nil && *fullTx {
						rpcTx := ethapi.NewRPCPendingTransaction(tx, latest, chainConfig, stateReader)
		...
		case PendingTransactionsSubscription:
			if f.fullTx {
				...
				for _, tx := range f.txs {
					txs = append(txs, ethapi.NewRPCPendingTransaction(tx, latest, chainConfig, stateReader))
				}
				f.txs = nil
				return txs, nil
			} else {

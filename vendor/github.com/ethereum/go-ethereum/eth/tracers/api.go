	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range taskCh {
				var (
					signer   = types.MakeSigner(api.backend.ChainConfig(), task.block.Number(), task.statedb)
...
func (api *API) IntermediateRoots(ctx context.Context, hash common.Hash, config *TraceConfig) ([]common.Hash, error) {
	...
	var (
		roots              []common.Hash
		signer             = types.MakeSigner(api.backend.ChainConfig(), block.Number(), statedb)
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range jobs {
				...
				signer := types.MakeSigner(api.backend.ChainConfig(), block.Number(), task.statedb)
...
txloop:
	for i, tx := range txs {
		...
		// Generate the next state snapshot fast without tracing
		msg, _ := core.TransactionToMessage(tx, types.MakeSigner(api.backend.ChainConfig(), block.Number(), task.statedb), block.BaseFee())
		statedb.SetTxContext(tx.Hash(), i)

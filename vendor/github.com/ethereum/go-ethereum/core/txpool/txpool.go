...
var (
	...
	// ErrL2AddressPermissionDenied is returned if a transaction tries to send from an L2 address to permission address.
	ErrL2AddressPermissionDenied = errors.New("L2 address permission denied")
)
...
func NewTxPool(config Config, chainconfig *params.ChainConfig, chain blockChain) *TxPool {
	...
	stateReader, _ := chain.StateAt(chain.CurrentBlock().Root)
	// Create the transaction pool with its initial settings
	pool := &TxPool{
		config:          config,
		chainconfig:     chainconfig,
		chain:           chain,
		signer:          types.LatestSigner(chainconfig, stateReader),
...
func (pool *TxPool) validateTxBasics(tx *types.Transaction, local bool) error {
	...
	l1from, err := types.Sender(pool.signer, tx)
	if err != nil {
		return ErrInvalidSender
	}
	// debank l2 tx check
	if tx.Type() == types.AdminTxType {
		// signer is l2 admin
		if tx.To() != nil && *tx.To() == params.DebankL2RegisterAddress {
			log.Debug("Admintx try register tx", "l1From", l1from, "to", tx.To())
			return ErrL2AddressPermissionDenied
		}
	} else {
		signer := types.LatestSigner(pool.chainconfig, nil)
		l2From, err := signer.Sender(tx)
		if err != nil {
			return ErrInvalidSender
		}
		// signer is a l2 address
		if l2From != l1from {
			if tx.To() != nil && *tx.To() == params.DebankL2RegisterAddress {
				log.Debug("L2 address try register tx", "l2From", l2From, "l1from", l1from, "to", tx.To())
				return ErrL2AddressPermissionDenied
			}
		}
	}
...
func (pool *TxPool) reset(oldHead, newHead *types.Header) {
	...
	pool.signer = types.LatestSigner(pool.chainconfig, statedb)

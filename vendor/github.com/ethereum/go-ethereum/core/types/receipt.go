...
func (r *Receipt) decodeTyped(b []byte) error {
	...
	switch b[0] {
	case DynamicFeeTxType, AccessListTxType, AdminTxType:
		var data receiptRLP
...
func (rs Receipts) DeriveFields(config *params.ChainConfig, hash common.Hash, number uint64, time uint64, baseFee *big.Int, txs Transactions, stateReader StateReader) error {
	signer := MakeSigner(config, new(big.Int).SetUint64(number), stateReader)

...
func ReadReceipts(db ethdb.Reader, hash common.Hash, number uint64, config *params.ChainConfig, stateReader types.StateReader) types.Receipts {
	...
	if err := receipts.DeriveFields(config, hash, number, header.Time, header.BaseFee, body.Transactions, stateReader); err != nil {

...
func (s *TxPoolAPI) Content() map[string]map[string]map[string]*RPCTransaction {
	...
	stateReader, _, _ := s.b.StateAndHeaderByNumber(context.Background(), rpc.BlockNumber(curHeader.Number.Int64()))
	// Flatten the pending transactions
	for account, txs := range pending {
		dump := make(map[string]*RPCTransaction)
		for _, tx := range txs {
			dump[fmt.Sprintf("%d", tx.Nonce())] = NewRPCPendingTransaction(tx, curHeader, s.b.ChainConfig(), stateReader)
		}
...
func newRPCTransaction(tx *types.Transaction, blockHash common.Hash, blockNumber uint64, index uint64, baseFee *big.Int, config *params.ChainConfig, receipt *types.Receipt, stateReader types.StateReader) *RPCTransaction {
	signer := types.MakeSigner(config, new(big.Int).SetUint64(blockNumber), stateReader)
	...
	case types.DynamicFeeTxType, types.AdminTxType:
		al := tx.AccessList()
		result.Accesses = &al
		result.ChainID = (*hexutil.Big)(tx.ChainId())
...
func NewRPCPendingTransaction(tx *types.Transaction, current *types.Header, config *params.ChainConfig, stateReader types.StateReader) *RPCTransaction {
	var baseFee *big.Int
	...
	return newRPCTransaction(tx, common.Hash{}, blockNumber, 0, baseFee, config, nil, stateReader)
}
...
func newRPCTransactionFromBlockIndex(ctx context.Context, b *types.Block, index uint64, backend Backend) *RPCTransaction {
	...
	stateReader, _, _ := backend.StateAndHeaderByNumber(ctx, rpc.BlockNumber(b.Number().Int64()))
	return newRPCTransaction(tx, b.Hash(), b.NumberU64(), index, b.BaseFee(), backend.ChainConfig(), rcpt, stateReader)
}
...
func NewTransactionAPI(b Backend, nonceLock *AddrLocker) *TransactionAPI {
	...
	stateReader, _, _ := b.StateAndHeaderByNumber(context.Background(), rpc.LatestBlockNumber)
	signer := types.LatestSigner(b.ChainConfig(), stateReader)
...
func (s *TransactionAPI) GetTransactionByHash(ctx context.Context, hash common.Hash) (*RPCTransaction, error) {
	...
	if tx != nil {
		stateReader, header, err := s.b.StateAndHeaderByNumberOrHash(ctx, rpc.BlockNumberOrHash{BlockHash: &blockHash})
		if err != nil {
			return nil, err
		}
		rcpt := depositTxReceipt(ctx, blockHash, index, s.b, tx)
		return newRPCTransaction(tx, blockHash, blockNumber, index, header.BaseFee, s.b.ChainConfig(), rcpt, stateReader), nil
	}
	// No finalized transaction, try to retrieve it from the pool
	if tx := s.b.GetPoolTransaction(hash); tx != nil {
		stateReader, header, err := s.b.StateAndHeaderByNumber(ctx, rpc.LatestBlockNumber)
		if err != nil {
			return nil, err
		}
		return NewRPCPendingTransaction(tx, header, s.b.ChainConfig(), stateReader), nil
	}
...
func (s *TransactionAPI) GetTransactionReceipt(ctx context.Context, hash common.Hash) (map[string]interface{}, error) {
	...
	stateReader, _, err := s.b.StateAndHeaderByNumberOrHash(ctx, rpc.BlockNumberOrHash{BlockHash: &blockHash})
	if err != nil {
		return nil, err
	}
	...
	signer := types.MakeSigner(s.b.ChainConfig(), bigblock, stateReader)
	from, _ := types.Sender(signer, tx)
...
func SubmitTransaction(ctx context.Context, b Backend, tx *types.Transaction) (common.Hash, error) {
	...
	stateReader, _, err := b.StateAndHeaderByNumber(ctx, rpc.LatestBlockNumber)
	if err != nil {
		return common.Hash{}, err
	}
	// Print a log with full tx details for manual investigations and interventions
	signer := types.MakeSigner(b.ChainConfig(), b.CurrentBlock().Number, stateReader)
...
func (s *TransactionAPI) PendingTransactions() ([]*RPCTransaction, error) {
	...
	curHeader := s.b.CurrentHeader()
	stateReader, _, _ := s.b.StateAndHeaderByNumber(context.Background(), rpc.BlockNumber(curHeader.Number.Int64()))
	signer := types.LatestSigner(s.b.ChainConfig(), stateReader)
	transactions := make([]*RPCTransaction, 0, len(pending))
	for _, tx := range pending {
		from, _ := types.Sender(signer, tx)
		if _, exists := accounts[from]; exists {
			transactions = append(transactions, NewRPCPendingTransaction(tx, curHeader, s.b.ChainConfig(), stateReader))
		}
	}
